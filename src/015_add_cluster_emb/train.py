...
def make_feature(train_df, test_df):
    ...
    gr_df = all_df.groupby("breath_id")["time_step"].nth(79).reset_index()
    X = all_df.groupby("breath_id")["time_step"].nth(79).values.reshape([-1, 1])
    model = KMeans(n_clusters=9, random_state=2021)
    model.fit(X)
    gr_df["time_step_cluster"] = model.labels_

    train_df = train_df.merge(
        gr_df[["time_step_cluster", "breath_id"]], on="breath_id", how="left"
    )
...
class PressureDataset(Dataset):
    def __init__(self, df, seq_features, is_train=True):
        ...
        self.time_cluster_dict = df.groupby("breath_id")["time_step_cluster"].first()
    ...
    def __getitem__(self, idx):
        ...
        cluster_value = self.time_cluster_dict[breath_id]

        global_features = torch.tensor(
            [r_value, c_value, cluster_value], dtype=torch.long)
...
class RNNModel(nn.Module):
    def __init__(
        ...
        self.time_cluter_emb = nn.Embedding(9, 8)
        ...
        self.decoder_out = nn.Sequential(
            nn.Linear(n_hidden * 2 + 8 * 3, n_hidden),
            nn.LayerNorm(n_hidden),
            nn.ReLU(),
    ...
    def __call__(self, features):
        ...
        global_hidden = torch.cat(
            [
                ...
                self.time_cluter_emb(features["global"][:, 2])
            ],
            dim=-1,
        )
        ...
        hidden = torch.cat(
            [hidden, global_hidden.unsqueeze(1).repeat([1, self.pred_len, 1])], dim=-1
        )
