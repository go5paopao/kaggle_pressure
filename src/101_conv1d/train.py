...
class RNNModel(nn.Module):
    def __init__(
        ...
        self.conv_layers = nn.Sequential(
            nn.Conv1d(self.seq_feature_len + 8 * 2, n_hidden//2, kernel_size=1),
            nn.ReLU(),
            nn.Conv1d(n_hidden//2, n_hidden, kernel_size=1),
        )
    ...
    def __call__(self, features):
        ...
        seq_hidden = self.conv_layers(seq_input.permute(0, 2, 1)).permute(0, 2, 1)
