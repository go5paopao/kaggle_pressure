...
def make_feature(train_df, test_df):
    def _make_feature_per_dataset_old(df):
    ...
    train_df["u_out_origin"] = train_df["u_out"].copy()
    test_df["u_out_origin"] = test_df["u_out"].copy()

    return train_df, test_df


def normalize_feature(train_df, valid_df, test_df, cols):

    # cols = [c for c in cols if c != "u_out"]
    ...
    scaler = RobustScaler()
    all_u_in = np.vstack([train_df[cols].values, valid_df[cols].values, test_df[cols].values])
    scaler.fit(all_u_in)
    train_df[cols] = scaler.transform(train_df[cols].values)
    valid_df[cols] = scaler.transform(valid_df[cols].values)
...
class RNNModel(nn.Module):
    def __init__(
        ...
        self.seq_linear = nn.Sequential(
            nn.Linear(self.seq_feature_len, 64),
            nn.LayerNorm(64),
            # nn.ReLU(),
...
def train_run(
    ...
    val_calc_indices = valid_df["u_out_origin"] == 0
...
class Config:
    ...
    n_hidden = 256
    USE_LAG = 4
    CONT_FEATURES = ['u_in', 'u_out', 'time_step'] \
    ...
    seq_features = CONT_FEATURES + LAG_FEATURES
    train_folds = [0]
...
def run():
    ...
    for fold_ix, (trn_idx, val_idx) in enumerate(
        ...
        _train_df, _valid_df, _test_df = normalize_feature(
            ...
            config.CONT_FEATURES + config.LAG_FEATURES
        )
