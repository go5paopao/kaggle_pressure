...
def make_feature(df):
    ...
    df = _make_feature_per_dataset(df)
...
def run():
    ...
    for fold_ix, (trn_idx, val_idx) in enumerate(
        ...
        # time_stepをそれぞれの中央値にする
        _train_df["time_idx"] = _train_df.index.values % 80
        _train_df["time_step"] = _train_df.groupby("time_idx")["time_step"].transform("median")

        _train_df = make_feature(_train_df)
        _valid_df = make_feature(_valid_df)
