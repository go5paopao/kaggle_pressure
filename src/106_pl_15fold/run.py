...
def postprocess(candidates, preds):
    result = np.zeros(len(preds))
    tk0 = tqdm(enumerate(preds), total=len(preds))
    for i, pred in tk0:
        res = candidates[idx_of_the_nearest(candidates, pred)]
        result[i] = res
    return result
...
def run():
    ...
    # postprocess
    candidates = sorted(oof_df.loc[oof_df['u_out'] == 0, 'pressure'].unique())

    postprocess_blended = postprocess(
        candidates, oof_df.loc[oof_df["u_out"] == 0, 'preds'].values)

    val_score = mean_absolute_error(
        oof_df.loc[calc_indices, "pressure"],
        postprocess_blended
    )
    logger.info(f"Val Score with PP: {val_score}")
    ...
    test_df["pressure"] = postprocess(candidates, test_df["pressure"])
