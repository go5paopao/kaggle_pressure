...
def run():
    ...
    train_aug_df = pd.read_csv(INPUT_DIR / "ventilator-pressure-prediction/train_aug1.csv")
    train_aug_df["u_in"] = train_aug_df["u_in"].clip(0)
    ...
    train_aug_df = make_feature(train_aug_df)
    test_df = make_feature(test_df)

    train_df["breath_id_ix"] = 0
    train_aug_df["breath_id_ix"] = 1
    train_df = pd.concat([train_df, train_aug_df], axis=0, ignore_index=True)
